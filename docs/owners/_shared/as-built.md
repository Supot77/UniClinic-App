...
| mock deterministic สำหรับ automated tests | test setup and repository adapters | `src/mocks/`, `src/features/mock-database/`, `src/features/scheduling/data/mockRepository.ts` | `tests/setup.ts`, mock repository tests | ทำแล้วใน code; adapters ไม่ถูก import โดย production runtime |
