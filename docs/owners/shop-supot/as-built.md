...
| department/doctor/service management | `/departments`, `DepartmentWorkspace` มีแท็บบริการ เพิ่ม/แก้ไข code, ชื่อ, คำอธิบาย และเปิด/ปิดสถานะ | `saveDepartment`, `toggleDepartment`, `saveDoctor`, `toggleDoctor`, `saveService`, `toggleService` ผ่าน `SchedulingRepository`; แก้บริการส่ง ID เดิม | `tests/database-scheduling-repository.test.ts`, migrations `13`, `17`, `/api/services` | ทำแล้วใน code; UI flow รอบนี้ยังไม่ได้รันทดสอบ component, DB/RLS ยังไม่ยืนยัน |
