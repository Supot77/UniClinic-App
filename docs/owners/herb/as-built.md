...
| dashboard medication/summary data | `DashboardScreen` | `getMedicationDashboardData`, `getDashboardView`, `getDashboardStats` | `tests/dashboard-service.test.ts` และ source ใน `src/services/dashboardService.ts` | ทำบางส่วน/ยังมี mock ตาม source ที่อ่าน |
...
`docs/02_user_stories.md` ระบุว่า dashboard metric บางส่วนใช้ mock repository ขณะที่ notifications/Broadcast ใช้ Supabase service/RPC ต้องแยกสถานะสองส่วนนี้
