...
| จัดการสถานะ reminder | `src/app/(patient)/reminders/page.tsx` | `src/services/reminderService.ts`: `pauseReminder`, `resumeReminder`, `completeReminder` | `src/services/reminderService.ts`, `docs/12_medication_reminders_updates.md`, owner confirmation กลอง | ทำแล้วใน code; DB/RLS ยังไม่ยืนยัน |
...
| สร้าง reminder จาก dispensing เต็มเท่านั้น | `src/app/(patient)/reminders/page.tsx`, `src/services/reminderService.ts` | ไม่พบหลักฐาน end-to-end enforcement จาก PAI/pharmacy ถึง reminder ใน active UI | `docs/02_user_stories.md`, owner confirmation กลอง | นอก scope ของ implementation ปัจจุบัน; ฟังก์ชันที่กลองรับผิดชอบถือว่าครบ |

## ข้อจำกัด

โค้ดมี pause/resume และ log missed ตาม scope ที่ owner ยืนยัน; worker/email/automation ไม่อยู่ใน scope. DB/RLS และ browser QA ยังไม่ใช่หลักฐานจาก owner confirmation นี้
