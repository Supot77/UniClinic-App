...
import { ALL_SERVICES_FILTER_VALUE, ALL_SERVICES_LABEL, isAllServicesLabel } from '@/constants/services';
import {
  actionLabels,
  allowedActions,
  bangkokDate,
...
function BookingForm({ data, busy, book, initialSlotId }: {
  ...
  const [serviceId, setServiceId] = useState(() => initialSlot && !isAllServicesLabel(initialSlot.service) ? initialSlot.service_id ?? ALL_SERVICES_FILTER_VALUE : ALL_SERVICES_FILTER_VALUE);
  ...
  const selectedService = serviceById.get(serviceId);
  const matchesSelectedService = (slot: ClinicSnapshot['slots'][number]) => serviceId === ALL_SERVICES_FILTER_VALUE
    || slot.service_id === serviceId
    || isAllServicesLabel(slot.service)
    || (!!selectedService && slot.service === selectedService.name);
  const slots = data.slots.filter((slot) => slot.slot_date === date && matchesSelectedService(slot) && isAvailableForPatient(slot));
  const selected = slots.find((slot) => slot.id === slotId);
  const services = data.services.filter((service) => !isAllServicesLabel(service.name));
  ...
  return <form data-appointment-booking="true" className="min-w-0 border-b border-brand-border-soft pb-8 sm:pb-10" onSubmit={async (event) => {
    ...
    <fieldset disabled={busy} className="grid min-w-0 gap-6 lg:grid-cols-2 lg:gap-10">
      <div className="min-w-0 space-y-5">
        <div className="grid min-w-0 gap-5 sm:grid-cols-2">
          ...
          <div className="min-w-0 text-sm font-semibold text-brand-ink">
            <p>บริการ</p>
            <div className="mt-2">
              <ClinicSelect value={serviceId} onChange={(value) => { setServiceId(value); setSlotId(''); }} placeholder={text(ALL_SERVICES_LABEL, 'All services')} ariaLabel={text('บริการ', 'Service')} className={bookingControlClass} options={[{ value: ALL_SERVICES_FILTER_VALUE, label: text(ALL_SERVICES_LABEL, 'All services') }, ...services.map((service) => ({ value: service.id, label: service.name }))]} />
            </div>
          </div>
        </div>
      ...
      <section aria-labelledby="appointment-slot-title" className="space-y-2">
        ...
        <ClinicSelect value={selected?.id ?? ''} onChange={setSlotId} placeholder={text('เลือกรอบตรวจ', 'Choose a slot')} ariaLabel={text('รอบตรวจ', 'Appointment slot')} className={bookingControlClass} options={slots.map((slot) => ({ value: slot.id, label: `${slot.start_time.slice(0, 5)}–${slot.end_time.slice(0, 5)} · ${slot.doctor} · ${text('มีรอบนี้', 'available')} · ${text(`ว่าง ${slot.max_capacity - slot.booked_count} ที่`, `${slot.max_capacity - slot.booked_count} available`)}` }))} />
...
function AppointmentCard({ appointment, slot, role, state, bangkokNow, onStartExam, onOpenExam }: {
  ...
  const actions = allowedActions(role, appointment, slot, bangkokNow.date, bangkokNow.time);
  ...
  return <article className={role === 'patient'
    ...
    {(actions.length > 0 || (role === 'medical' && appointment.status === 'confirmed') || (role === 'medical' && appointment.status === 'in_progress') || (role === 'patient' && appointment.status === 'completed') || (role === 'staff_admin' && appointment.has_record)) && <div className={`${role === 'patient' ? 'mt-auto' : 'mt-5'} flex flex-wrap items-start gap-2 border-t border-brand-border-soft pt-4`}>
      ...
      {((role === 'medical' && appointment.status === 'in_progress' && (appointment.has_record || !onOpenExam)) || (role === 'staff_admin' && appointment.has_record)) && <Link href={`/records?appointment=${appointment.id}`} className={`${primaryButtonClass} rounded-xl`}><FileText className="h-4 w-4" aria-hidden="true" />เปิดผลตรวจ<ArrowUpRight className="h-4 w-4" aria-hidden="true" /></Link>}
...
export default function AppointmentPage({ role, repository, initialSlotId }: { role: ClinicRole; repository?: ClinicRepository; initialSlotId?: string }) {
  ...
  const rows = data?.appointments.filter((appointment) => {
    ...
    const matchesStatus = !status ? true : status === 'pending_confirmed' ? ['pending', 'confirmed'].includes(appointment.status) : status === 'cancel_requested' ? true : appointment.status === status;
    const matchesCancellationRequest = status === 'cancel_requested'
      ? Boolean(appointment.cancel_requested_at) && ['pending', 'confirmed'].includes(appointment.status)
      : true;
    const searchable = `${appointment.patient} ${appointment.patient_phone ?? ''} ${slot?.doctor ?? ''} ${slot?.department ?? ''} ${appointment.queue_number ?? ''} ${appointment.reason ?? ''}`;
    return (!date || slot?.slot_date === date) && matchesStatus && matchesCancellationRequest && searchable.toLowerCase().includes(query.toLowerCase());
  }).sort((left, right) => {
  ...
  const stats = data ? {
    ...
    in_progress: data.appointments.filter((appointment) => appointment.status === 'in_progress').length,
  ...
  const headerStats: WorkspaceHeaderStat[] = stats ? [
    ...
    { label: 'กำลังตรวจ', value: stats.in_progress, tone: 'info' },
    ...
    {state.loading ? <ClinicPageLoading /> : data && <div className={role === 'patient' ? 'space-y-8' : 'space-y-5'}>
      ...
      <section className="overflow-visible rounded-[1.75rem]">
        ...
        <div className="grid items-start gap-4 border-y border-brand-border-soft py-4 sm:grid-cols-2 lg:grid-cols-[minmax(0,2fr)_repeat(3,minmax(0,1fr))]">
          ...
          <div className="flex min-w-0 flex-col text-sm"><p className="min-h-5 font-medium leading-5 text-brand-body">สถานะนัดหมาย</p><ClinicSelect value={status} onChange={setStatus} placeholder="ทุกสถานะ" ariaLabel="สถานะ" className="!mt-2 !h-12 !border-brand-border" options={[...(role === 'staff_admin' ? [{ value: 'cancel_requested', label: 'คำขอยกเลิก' }] : []), ...(role === 'medical' ? [{ value: 'pending_confirmed', label: 'รออนุมัติและรอตรวจ' }] : []), ...Object.entries(statusLabels).map(([value, label]) => ({ value, label }))]} /></div>
