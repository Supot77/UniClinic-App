...
function RecordList({ data, selectedId }: { data: ClinicSnapshot; selectedId?: string }) {
  ...
  return <section className="space-y-4">
    <div className="flex flex-wrap items-end justify-between gap-3"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600"><ClipboardCheck className="h-5 w-5" aria-hidden="true" /></span><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-brand-strong">MEDICAL HISTORY</p><h2 className="mt-1 text-lg font-bold text-brand-ink">{data.actor.role === 'patient' ? 'ประวัติผลตรวจของฉัน' : 'ผลตรวจและประวัติผู้ป่วยที่รับผิดชอบ'}</h2><p className="mt-1 text-xs text-brand-body">แสดงเฉพาะรายการที่บัญชีนี้มีสิทธิ์เข้าถึง</p></div></div><span className="rounded-full bg-brand-surface px-3 py-1.5 text-xs font-bold text-brand-strong">{records.length} รายการ</span></div>
...
function RecordsIntro({ role, records, pending }: { role: 'patient' | 'medical'; records: number; pending?: number }) {
  return <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-brand-strong">{role === 'medical' ? 'CLINICAL RECORDS / MEDICAL' : 'CLINICAL RECORDS / PATIENT'}</p><h1 className="mt-1 text-2xl font-bold tracking-tight text-brand-ink sm:text-3xl">{role === 'medical' ? 'ผลตรวจและรายการยา' : 'ประวัติการรักษา'}</h1><p className="mt-1 max-w-2xl text-sm text-brand-body">{role === 'medical' ? 'บันทึกผลตรวจอย่างเป็นระบบ และส่งต่อข้อมูลที่จำเป็นให้ผู้ป่วย' : 'เปิดดูผลตรวจ การตรวจร่างกาย และรายการยาของคุณในที่เดียว'}</p></div><div className="flex gap-2 text-xs font-semibold text-brand-body"><span className="rounded-full bg-brand-surface px-3 py-1.5">{records} ประวัติ</span>{pending !== undefined && <span className="rounded-full bg-amber-50 px-3 py-1.5 text-amber-800">{pending} คิวรอบันทึก</span>}</div></div>;
}
...
export function PatientRecordsPage({ repository, selectedId }: { repository?: ClinicRepository; selectedId?: string }) {
  ...
  return <ClinicWorkspaceShell {...state} role="patient" section="records">{state.loading ? <ClinicPageLoading /> : state.data && <div className="space-y-5"><RecordsIntro role="patient" records={state.data.records.length} /><RecordList data={state.data} selectedId={selectedId} /></div>}</ClinicWorkspaceShell>;
}
