...
export function LandingActions() {
  ...
  if (isLoading) return <p role="status" className="mt-7 min-h-12 text-sm text-brand-muted">กำลังตรวจสอบการเข้าสู่ระบบ…</p>;
  ...
  return <div className="mt-7 flex flex-wrap items-center gap-3">
    <Link href={staff ? dashboardPathForRole(role) : isAuthenticated ? "/appointments" : "/login?redirect=%2Fappointments"} className={`inline-flex min-h-12 items-center justify-center gap-3 rounded-brand-button bg-brand-ink px-6 py-3 text-sm font-semibold text-white hover:bg-brand-hover ${focus}`}>
      {staff ? "ไป Dashboard" : "จองนัดหมาย"}<ArrowRight size={18} aria-hidden="true" />
    </Link>
    <Link href={isAuthenticated ? "/schedules" : "/login?redirect=%2Fschedules"} className={`inline-flex min-h-12 items-center justify-center rounded-brand-button border border-brand-border-strong px-5 py-3 text-sm font-semibold hover:bg-white ${focus}`}>ดูตารางบริการ</Link>
...
export function LandingServices() {
  ...
  if (status === "loading") return <p role="status" className="mt-8 border-t border-brand-border py-8 text-brand-muted">กำลังโหลดบริการ…</p>;
  if (status === "error" || services.length === 0) return <div className="mt-8 border-t border-brand-border py-6"><p role="status" className="text-brand-body">{status === "error" ? "โหลดรายการบริการไม่สำเร็จ กรุณาลองอีกครั้ง" : "ยังไม่มีรายการบริการที่แสดงได้ กรุณาตรวจสอบบริการและรอบตรวจหลังเข้าสู่ระบบ"}</p><button type="button" onClick={() => { setStatus("loading"); setAttempt((value) => value + 1); }} className={`mt-3 min-h-11 text-sm font-semibold text-brand-strong underline underline-offset-4 ${focus}`}>ลองโหลดบริการอีกครั้ง</button></div>;
