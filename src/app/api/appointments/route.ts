...
export async function GET() {
  ...
  const select = 'id, patient_id, slot_id, queue_number, reason, status, cancel_requested_at, rejection_reason, created_at, updated_at, slot:appointment_slots(id, doctor_id, daily_service_offering_id, slot_date, start_time, end_time, max_capacity, booked_count, status, offering:daily_service_offerings(service_id, service:services(id, code, name)), doctor:doctors(id, specialty, department_id, profile:profiles(id, title, first_name, last_name), department:departments(id, name))), patient:profiles(id, title, first_name, last_name, phone)';
