...
type RecordInput = {
  appointmentId?: string; appointment_id?: string; diagnosis?: string; advice?: string; treatment_notes?: string;
  prescriptions?: Prescription[]; prescribed_medications?: Prescription[]; complete?: boolean;
  height_cm?: number | null; weight_kg?: number | null; blood_pressure?: string | null; pulse_bpm?: number | null;
};

function validPhysicalExam(body: RecordInput): string | null {
...
export async function POST(request: Request) {
  ...
  const appointmentId = parseUuid(body.appointmentId ?? body.appointment_id);
  ...
  if (!appointmentId || !diagnosis || diagnosis.length > 5000 || advice.length > 5000 || !Array.isArray(prescriptions) || prescriptions.length > 50) return Response.json({ error: 'ข้อมูลผลตรวจไม่ครบถ้วนหรือยาวเกินกำหนด' }, { status: 400 });
  ...
  for (const item of prescriptions) {
    ...
    if (!medicationId || !item.name?.trim() || !item.dosage?.trim() || !item.frequency?.trim() || !parsePositiveInt(item.quantity) || !parsePositiveInt(item.duration_days) || seen.has(medicationId)) return Response.json({ error: 'รายการยาไม่ถูกต้องหรือซ้ำกัน' }, { status: 400 });
    seen.add(medicationId);
  }
  const { data, error } = await auth.supabase.rpc('pai_save_record', {
  ...
  if (error) return errorResponse(error, 'บันทึกผลตรวจไม่สำเร็จ');
